...
pub const NUM_TOKEN_TYPES: usize =
    (NUM_CORNERS*NUM_CORNER_ROT) + (NUM_EDGES*NUM_EDGE_ROT) + 1 /* Masked-out token */;
...
pub fn cube_to_tokens(cube_state: &CubeState, mask: &CubeMask) -> MLStateTokens {
    ...
    for i in 0..NUM_CORNERS {
        if mask.corners[i] {
            let corner_perm = cube_state.corn_perm[i];
            let corner_rot = cube_state.corn_rot[i];
            let base_token_idx =
                ((corner_perm as usize) * NUM_CORNER_ROT) + (corner_rot as usize);
    ...
    for i in 0..NUM_EDGES {
        if mask.edges[i] {
            let edge_perm = cube_state.edge_perm[i];
            let edge_rot = cube_state.edge_rot[i];
            let base_token_idx =
                ((edge_perm as usize) * NUM_EDGE_ROT) + (edge_rot as usize);
            tokens.push(1 + (NUM_CORNERS*NUM_CORNER_ROT) as u16 + base_token_idx as u16);
        } else {
            tokens.push(0);
        }
    }
